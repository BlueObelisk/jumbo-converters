...
//		PREFIX_MAP.put(CIFConstants.IUCR_PREFIX, CIFConstants.IUCR_URI_HASH);
   ...
   private void loadOntology(String ontologyResourceName) {
      if (ontology == null) {
         try {
            ontologyURI = getClass().getClassLoader().getResource(
                    ontologyResourceName).toURI();
            manager = OWLManager.createOWLOntologyManager();
            ontology = manager.loadOntologyFromPhysicalURI(ontologyURI);
         } catch (Exception e) {
            throw new RuntimeException(
                    "Cannot load ontology: " + ontologyResourceName, e);
         }
...
//		CMLUtil.debug(rdfXml, "RDF");
   ...
   private void processElement(Resource resource, CMLElement cmlElement,
      ...
      if (--count < 0) {
         throw new RuntimeException(
                 "uncontrolled recursion stopped at " + cmlElement);
      }
      if (false) {
   ...
   private void createAndAddTripleFromId(Resource resource,
      ...
      Property property = rdfModel.createProperty(RDFUtils.cleanAndMakeURI(
              RDFUtils.CML_URI_HASH, "hasId"));
      String id = cmlElement.getId();
      if (id != null) {
//			System.out.println("ID "+id);
         RDFUtils.add(rdfModel, resource, property, RDFUtils.
                 removeNonURLCharacters(id));
      }
   }

   private void createAndAddTripleFromAttribute(Resource resource,
           Attribute attribute) {
      if (attribute != null) {
         String namespaceURI = attribute.getNamespaceURI();
         String localName = attribute.getLocalName();
         if (!"id".equals(localName)) {
            String propertyName = RDFUtils.cleanAndMakeURI(namespaceURI,
                                                           localName);
      ...
      if (childElements.size() > 0) {
         ...
         if (content != null && !content.trim().equals(CMLConstants.S_EMPTY)) {
            Property property = rdfModel.createProperty(
                    RDFUtils.cleanAndMakeURI(RDFUtils.CML_URI_HASH,
                                             RDFUtils.HAS_VALUE));
            if (property != null) {

//			    	System.out.println("CONT "+content);
               RDFUtils.add(rdfModel, resource, property, rdfModel.
    ...
    * @param cmlElement
    */
   private void processDictionary(Resource resource, CMLElement cmlElement,
           int count) {
      resource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(resource, cmlElement, count - 1);
      createAndAddTripleFromAttribute(resource, cmlElement.getAttribute(
              "namespace"));
   }
    ...
    */
   private Resource createResourceAndAddToSubject(Resource resource,
           CMLElement cmlElement) {
...
//		LOG.debug("URI"+uri);
   ...
   private String getCMLProperty(CMLElement cmlElement) {
      String propertyName = RDFUtils.CML_URI_HASH + "has" + Util.capitalise(cmlElement.
              getLocalName());
      return propertyName;
   }
 ...
 /**
    ...
    * @param cmlElement
    */
   private void processFormula(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processIdentifier(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processMetadataList(Resource resource, CMLElement cmlElement,
           int count) {
      processChildren(resource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processMetadata(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processMolecule(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processParameter(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processProperty(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void processScalar(Resource resource, CMLElement cmlElement,
           int count) {
      Resource newResource = createResourceAndAddToSubject(resource, cmlElement);
      addIDAttributesAndChildren(newResource, cmlElement, count - 1);
   }
    ...
    * @param cmlElement
    */
   private void addIDAttributesAndChildren(Resource resource,
    ...
    * @param cmlElement
    */
   private void addAttributesAndChildren(Resource resource,
           CMLElement cmlElement, int count) {
      for (int i = 0; i < cmlElement.getAttributeCount(); i++) {
         ...
         if (attName.equals("id")) {
            ...
            Resource entryClass = rdfModel.createResource(
                    prefix + CMLConstants.S_COLON + entryId);
