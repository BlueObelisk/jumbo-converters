...
public class PubchemXML2CMLConverter extends AbstractConverter implements
	...
	public final static String[] typicalArgsForConverterCommand = {
		"-sd", "src/test/resources/pubchem",
		"-odir", "../temp",
		"-is", "xml",
		"-os", "cml",
		"-converter", "org.xmlcml.cml.converters.molecule.pubchem.PubchemXML2CMLConverter"
	};
...
//		"-quiet",
	...
	private void convert(Element pubchemXML) {
		...
		compoundId = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_id", PubchemUtils.NIH_XPATH);
		compoundAtoms = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_atoms", PubchemUtils.NIH_XPATH);
		compoundBonds = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_bonds", PubchemUtils.NIH_XPATH);
		compoundStereo = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_stereo", PubchemUtils.NIH_XPATH);
		compoundCoords = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_coords", PubchemUtils.NIH_XPATH);
		compoundCharge = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_charge", PubchemUtils.NIH_XPATH);
		compoundProps = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_props", PubchemUtils.NIH_XPATH);
		compoundCount = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_count", PubchemUtils.NIH_XPATH);
 ...
 */
		...
		Nodes nodes = cml.query("//cml:property[@*='IUPAC Name' and cml:scalar]", CMLConstants.CML_XPATH);
		for (int i = 0; i < nodes.size(); i++) {
			CMLProperty property = (CMLProperty) nodes.get(i);
			CMLScalar scalar = (CMLScalar) property.query("./cml:scalar", CMLConstants.CML_XPATH).get(0);
			String name = scalar.getValue();
			String pubchemName = property.getAttributeValue("name", PubchemUtils.NIH_NS);
			pubchemName = pubchemName.replaceAll(CMLConstants.S_SPACE, CMLConstants.S_EMPTY);
	...
	private void addInfoData() {
		Element propElement = CMLUtil.getSingleElement(pcCompound, "p:PC-Compound_props", PubchemUtils.NIH_XPATH);
		if (propElement != null) {
			Nodes infoDataNodes = propElement.query("p:PC-InfoData", PubchemUtils.NIH_XPATH);
  ...
  */
		Nodes chargeNodes = pcCompound.query("p:PC-Compound_charge", PubchemUtils.NIH_XPATH);
...
*/
		Element coordinatesElement = CMLUtil.getSingleElement(
				pcCompound, "p:PC-Compound_coords/p:PC-Coordinates", PubchemUtils.NIH_XPATH);
...
...
  ...
  */
		if (compoundStereo != null) {
			Nodes stereoCenterNodes = compoundStereo.query("p:PC-StereoCenter", PubchemUtils.NIH_XPATH);
	...
	private void processStereoCenter(Element stereoCenter) {
		...
		Element stereoCenterTetrahedral = CMLUtil.getSingleElement(
				stereoCenter, "p:PC-StereoCenter_tetrahedral", PubchemUtils.NIH_XPATH);
	...
	private void processTetrahedral(Element stereoCenter) {
		...
		Element above = CMLUtil.getSingleElement(stereoCenter, "p:PC-StereoTetrahedral_above", PubchemUtils.NIH_XPATH);
		Element top = CMLUtil.getSingleElement(stereoCenter, "p:PC-StereoTetrahedral_top", PubchemUtils.NIH_XPATH);
		Element bottom = CMLUtil.getSingleElement(stereoCenter, "p:PC-StereoTetrahedral_bottom", PubchemUtils.NIH_XPATH);
		Element below = CMLUtil.getSingleElement(stereoCenter, "p:PC-StereoTetrahedral_below", PubchemUtils.NIH_XPATH);
		String parity = CMLUtil.getSingleValue(stereoCenter, "p:PC-StereoTetrahedral_parity/@value", PubchemUtils.NIH_XPATH);
		@SuppressWarnings("unused")
...
...
   ...
   */		
		if (compoundBonds != null) {
			Element bondsElement = CMLUtil.getSingleElement(compoundBonds, "p:PC-Bonds", PubchemUtils.NIH_XPATH);
...
//				Element orderElement = CMLUtil.getSingleElement(bondsElement, "p:PC-Bonds_order", PubchemUtils.NIH_XPATH);
				...
				Nodes aid1Nodes = bondsElement.query("p:PC-Bonds_aid1/p:PC-Bonds_aid1_E", PubchemUtils.NIH_XPATH);
				Nodes aid2Nodes = bondsElement.query("p:PC-Bonds_aid2/p:PC-Bonds_aid2_E", PubchemUtils.NIH_XPATH);
				Nodes orderNodes = bondsElement.query("p:PC-Bonds_order/p:PC-BondType", PubchemUtils.NIH_XPATH);
	...
	private void addAtoms() throws RuntimeException {
		...
		*/
		...
		Element atomsElement = CMLUtil.getSingleElement(compoundAtoms, "p:PC-Atoms", PubchemUtils.NIH_XPATH);
		PubchemUtils.checkUnknowns(compoundAtoms, new String[] {
			"PC-Atoms",
		});
		if (atomsElement != null) {
			Element aidElement = CMLUtil.getSingleElement(atomsElement, "p:PC-Atoms_aid", PubchemUtils.NIH_XPATH);
			Element elemElement = CMLUtil.getSingleElement(atomsElement, "p:PC-Atoms_element", PubchemUtils.NIH_XPATH);
			Element isotopeElement = CMLUtil.getSingleElement(atomsElement, "p:PC-Atoms_isotope", PubchemUtils.NIH_XPATH);
			Element chargeElement = CMLUtil.getSingleElement(atomsElement, "p:PC-Atoms_charge", PubchemUtils.NIH_XPATH);
			...
			if (aidElement != null && elemElement != null) {
				Nodes aidNodes = aidElement.query("p:PC-Atoms_aid_E", PubchemUtils.NIH_XPATH);
				int size = aidNodes.size();
				Nodes elemNodes = elemElement.query("p:PC-Element", PubchemUtils.NIH_XPATH);
				...
				for (int i = 0; i < aidNodes.size(); i++) {
					...
					atom.setId(id);
					String value = ((Element)elemNodes.get(i)).getAttributeValue("value");
 ...
 */		
		...
		Nodes atomIntNodes = chargeElement.query("p:PC-AtomInt", PubchemUtils.NIH_XPATH);
		for (int i = 0; i < atomIntNodes.size(); i++) {
			Element atomInt = (Element) atomIntNodes.get(i);
			Element atomIntAid = CMLUtil.getSingleElement(atomInt, "p:PC-AtomInt_aid", PubchemUtils.NIH_XPATH);
			Element atomIntValue = CMLUtil.getSingleElement(atomInt, "p:PC-AtomInt_value", PubchemUtils.NIH_XPATH);
 ...
 */					
		Nodes atomIntNodes = isotopeElement.query("p:PC-AtomInt", PubchemUtils.NIH_XPATH);
		for (int i = 0; i < atomIntNodes.size(); i++) {
			Element atomInt = (Element) atomIntNodes.get(i);
			Element atomIntAid = CMLUtil.getSingleElement(atomInt, "p:PC-AtomInt_aid", PubchemUtils.NIH_XPATH);
			Element atomIntValue = CMLUtil.getSingleElement(atomInt, "p:PC-AtomInt_value", PubchemUtils.NIH_XPATH);
	...
	private void addMoleculeId() throws RuntimeException {
		...
		*/
		...
		Element compoundType = CMLUtil.getSingleElement(compoundId, "p:PC-CompoundType", PubchemUtils.NIH_XPATH);
		...
		Element compoundTypeId = CMLUtil.getSingleElement(compoundType, "p:PC-CompoundType_id", PubchemUtils.NIH_XPATH);
		...
		Element compoundTypeIdCid = CMLUtil.getSingleElement(compoundTypeId, "p:PC-CompoundType_id_cid", PubchemUtils.NIH_XPATH);
