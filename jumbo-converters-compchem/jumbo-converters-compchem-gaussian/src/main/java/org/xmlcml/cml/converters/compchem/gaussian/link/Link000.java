...
public class Link000 extends GaussianLink {
	
    public Link000(GaussianLink glink, CMLMolecule molecule) {
		super(glink, molecule);
	...
	public CMLModule convert2CML() {
        ...
        while (line_num < lineList.size()) {
            ...
            /*if (line.indexOf("-- Stationary point found") >= 0) {
                // Reading the gaussian optimised geometry is not important at this point.
            } else if (line.indexOf("Input orientation:") >= 0 ||
                       line.indexOf("Z-Matrix orientation:") >= 0 ||
                       line.indexOf("Standard orientation:") >= 0) {
                // Reading geometry table is not neccessary since it has been read by converting
                // to mol file and being read by MolReader class.
            } else if (line.indexOf("Optimized Parameters") >= 0 ||
                       line.indexOf("Initial Parameters") >= 0) {
                // Connectivity table which does not give you all the information.
            } else if (line.startsWith(" Energy=")) {
                System.out.println(line_num + " Found:  Energy=");
            } else if (line.startsWith(" Normal termination of Gaussian")) {
                System.out.println(line_num + " Found:  Normal termination of Gaussian");
            } else*/
...
//        RotationalConstants rotConst = new RotationalConstants();
...
//        mDoc.setRotationalConstants(rotConst);
...
//        mDoc.setRotationalSymmetryNumber((int) Math.round(Double.parseDouble(sym_str)));
	@SuppressWarnings("unused")
